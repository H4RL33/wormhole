...
- **Local API Socket**: The `wormholed` local API uses `net.Listen("unix", ...)` with OS-default socket permissions and no additional bearer-token layer or `chmod` hardening. Any process able to connect is inside the local trust boundary; registration returns that caller's newly issued raw token once over this socket. The intended same-user model therefore depends on restrictive OS permissions for the socket path and parent directory (RFC-0003 OQ4), which users must verify.
