...
type routedSyncRepositories struct {
	...
	audit      *syncpkg.AuditRepo
}

func newRoutedSyncRepositories(store *localstore.Store) (routedSyncRepositories, error) {
	if store == nil {
		return routedSyncRepositories{}, fmt.Errorf("construct routed sync repositories: local store is required")
	}
	workspaces := localstore.NewWorkspaceRepo(store.DB())
	return routedSyncRepositories{
