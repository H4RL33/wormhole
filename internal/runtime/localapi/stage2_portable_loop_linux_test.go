...
func assertFreshClonePrivateInventory(t *testing.T, store *localstore.Store, binding types.WorkspaceBinding, root string) {
	t.Helper()
	wantTables := []string{
		"activity_cursors", "activity_ingress_receipts", "activity_ledger", "activity_lifecycle", "activity_outbound_queue", "activity_policy_current", "activity_policy_versions", "activity_promotion_receipts",
	...
	if !reflect.DeepEqual(gotTables, wantTables) {
		t.Fatalf("schema-v8 private table inventory = %v, want %v", gotTables, wantTables)
	}
