...
func proveCheckpointPlan(input checkpointPlanInput) (checkpointPlan, error) {
	...
	if _, err := proveCurrentMaterializationWorkset(input.Disposition); err != nil {
		return checkpointPlan{}, fmt.Errorf("projectstate: checkpoint current materialization workset: %w", err)
	}
