...
func TestAlphaAcceptancePullAppliesEventAndGitStableIDsExactlyOnce(t *testing.T) {
	...
	queue := NewQueueRepo(store.DB())
	bindRoutedTestQueue(t, store, queue)
	engine := mustNewEngine(t, "http://fabric.invalid", queue, NewAuditRepo(store.DB()), tasks, kb, DefaultConfig())
	...
	updates := []map[string]any{
		...
		{"type": "channel", "data": map[string]any{"id": "channel-1", "project_id": routedTestProjectID, "name": "acceptance", "created_at": now}},
		{"type": "event", "data": map[string]any{"id": "event-1", "project_id": routedTestProjectID, "channel_id": "channel-1", "agent_id": "agent-a", "event_type": "discovery.logged", "payload": map[string]any{"finding": "stable"}, "note": "handoff", "created_at": now}},
		{"type": "git_link", "data": map[string]any{"git_link_id": "git-1", "project_id": routedTestProjectID, "task_id": "task-1", "repo": "H4RL33/wormhole", "commit_sha": "abc123", "summary": "handoff", "agent_id": "agent-a", "created_at": now}},
	}
	...
	if got, err := events.GetChannel(ctx, routedTestProjectID, "channel-1"); err != nil || got != "acceptance" {
		t.Fatalf("stable channel = %q, err %v", got, err)
	}
	gotEvent, err := events.GetEvent(ctx, routedTestProjectID, "event-1")
...
func TestAlphaAcceptanceUnknownOrMalformedPullNeverAdvancesCursor(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			queue := NewQueueRepo(store.DB())
			bindRoutedTestQueue(t, store, queue)
			engine := mustNewEngine(t, "http://fabric.invalid", queue, NewAuditRepo(store.DB()),
				localstore.NewTaskRepo(store.DB(), events), localstore.NewKBRepo(store.DB()), DefaultConfig())
