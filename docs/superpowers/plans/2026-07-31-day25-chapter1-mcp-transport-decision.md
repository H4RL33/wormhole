# Chapter 1 — MCP Transport & Auth Decision Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Decide and document the real MCP wire transport (JSON-RPC 2.0 over Streamable HTTP) and the auth carry-over strategy that M1's later chapters (2-4) implement against, per `ROADMAP-ALPHA2.md` Chapter 1.

**Architecture:** This is a documentation-only chapter — no Go code changes. It produces `docs/mcp-protocol.md`, the design-of-record that Chapters 2-4 implement mechanically. All schema decisions here are explicitly flagged as inference (RFC-0001 §9 marks the MCP tool surface "indicative, not finalised").

**Tech Stack:** Markdown only. No code, no tests, no build.

## Global Constraints

- Authority order: RFC-0001 > RFC-0002 > `docs/architecture.md` > existing code (per `docs/architecture.md` header and project `CLAUDE.md`).
- RFC-0001 §9 (M1 in `docs/architecture.md` §5): tool *names* are fixed (`wormhole.<pillar>.<verb>`), exact request/response schemas are decided at implementation time and must be frozen in `internal/mcp` — this doc is where that freeze happens for the transport envelope.
- `docs/architecture.md` §5 M2: naming grammar `wormhole.<pillar-noun>.<verb>`, pillars `agent`, `channel`, `task`, `kb`, `git`. No new pillar prefixes.
- `docs/architecture.md` §5 M4: auth happens at the MCP boundary (`internal/mcp` middleware resolves bearer token via `identity.Store.WhoAmI`), core packages receive an already-resolved scope, never a raw token.
- Existing bespoke shape to replace (read, do not re-derive): `internal/mcp/server.go` — `CallRequest{Tool, ProjectID, Arguments}`, `CallResponse{Result, Error}`, `bearerToken()` strips `Bearer ` prefix from `Authorization` header, `NewCallHandler` resolves auth via `identityStore.WhoAmI` before calling `tool.Handler`.
- No new external Go dependency decisions in this chapter (R4 in `docs/architecture.md` — an MCP SDK choice, if any, is Chapter 2+'s decision, not this one's).
- Do not resolve RFC-0001 §15 open questions (e.g. push vs poll delivery) as a side effect of writing this doc.

---

### Task 1: Write `docs/mcp-protocol.md` — transport shape and auth carry-over decision

**Files:**
- Create: `docs/mcp-protocol.md`

**Interfaces:**
- Consumes: nothing (first artifact in the chain).
- Produces: the frozen JSON-RPC envelope, method names, and auth header contract that Chapter 2 (`internal/mcp/jsonrpc.go`, `initialize`, `tools/list` handlers) and Chapter 3 (`tools/call` handler, `/mcp` endpoint replacing `/mcp/tools` + `/mcp/tools/call`) implement literally. Field names and error codes below are load-bearing for those chapters — do not paraphrase them when Chapter 2/3 briefs are written later.

- [ ] **Step 1: Write the full document**

Create `docs/mcp-protocol.md` with exactly this content:
...
- [ ] **Step 2: Verify the document against RFC-0001 §9 and `docs/architecture.md` §5**

Re-read RFC-0001 §9 (`docs/rfcs/wormhole_rfc.md` lines 233-261) and `docs/architecture.md` §5
(MCP Surface Rules) side by side with the file just written. Confirm:
- Every tool name referenced (`wormhole.task.create`, `wormhole.agent.whoami`) matches RFC-0001
  §9's naming exactly — no invented tool names.
- The doc does not claim any MCP schema is final (M1 rule: "indicative, not finalised") —
  it must read as a decision *this chapter is entitled to make*, not as an RFC quote.
- The doc does not resolve an RFC-0001 §15 open question. Re-check the list at
  `docs/rfcs/wormhole_rfc.md` (open questions section) — confirm push/poll delivery is not
  touched (it isn't; §2's SSE stub explicitly defers it).

If any mismatch is found, fix the file before proceeding — do not commit a doc that
contradicts the RFC it cites.

- [ ] **Step 3: Commit**

```bash
git add docs/mcp-protocol.md
git commit -m "docs: decide MCP JSON-RPC transport and auth carry-over (Chapter 1)"
```
...
- Spec coverage: ROADMAP-ALPHA2.md Chapter 1 has exactly two checklist items — transport
  decision + doc, and auth carry-over decision. Both are covered in Task 1 (§§2-4 for
  transport, §5 for auth) inside the single required output file `docs/mcp-protocol.md`.
  One task, not two, because both decisions land in the same file with no independent
  reviewable seam between them (splitting would force the reviewer to re-read the whole file
  twice for one coherent design doc).
- No code changes in this chapter — Task 1 has no test step because there is nothing to
  execute; Step 2 substitutes a citation-check review step, matching `docs/architecture.md`
  §0.7's "evidence before done" for a documentation deliverable.
